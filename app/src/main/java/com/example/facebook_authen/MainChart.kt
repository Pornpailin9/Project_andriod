...
class MainChart : Fragment() {
    ...
    fun Bar_Chart( chart : BarChart ){
        ...
        //สุ่มข้อมูล 12 อัน
        val listCookieR = CookieR.getSampleCookieRData(5)

        //สร้าง ArrayList<BarEntry> ขึ้นมาเพื่อเก็บข้อมูลนักเรียน และใส่ข้อมูล ลงไปใน entry แต่ละตัว
        val entries: ArrayList<BarEntry> = ArrayList()
        var index : Float = 0F
        for (CookieR in listCookieR) {
            entries.add(BarEntry(index, CookieR.score))
            index++
        }
...
//        xAxis.setCenterAxisLabels(false)
        ...
        xAxis.setValueFormatter(object : ValueFormatter() {
            ...
            override fun getFormattedValue(value: Float): String? {
                ...
                if( value < 0 || value >= listCookieR.size ){
                    return ""
                }
                else{
                    return listCookieR[value.toInt()].name
                }
...
//        rightAxis.axisMinimum = 0f // this replaces setStartAtZero(true)
    ...
    fun Pie_Chart( chart: PieChart){
        ...
        for (CookieR in listCookieR) {
            entries.add(PieEntry(CookieR.score, CookieR.name))
        }

        val dataset = PieDataSet(entries, " : CookieRun")
...
//        dataset.setXValuePosition(PieDataSet.ValuePosition.OUTSIDE_SLICE)
        ...
        chart.setCenterText("My CookieRun");
    ...
    fun Line_Chart(chart : LineChart) {
        ...
        val dataSet = LineDataSet(entries, "Number of cookie run")
        ...
        val months = arrayOf("January", "February", "March", "April", "March")
